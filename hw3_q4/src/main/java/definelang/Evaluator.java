...
public class Evaluator implements Visitor<Value> {
    ...
    public Value visit(LetExp e, Env env) { // New for varlang.
        ...
        for (int i = names.size() - 1; i >= 0; i--) {
            Value val = (Value) value_exps.get(i).accept(this, current_env);
            current_env = new ExtendEnv(current_env, names.get(i), val);
        }

        return (Value) e.body().accept(this, current_env);
    }
