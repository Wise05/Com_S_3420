...
public class Evaluator implements Visitor<Value> {
    ...
    public Value visit(CallExp e, Env env) {
        int currentDepth = env.depth();
        if (currentDepth > MAX_DEPTH) {
          throw new RuntimeException("Warning: Potential infinite recursion.");
        }
        ...
        for (int index = 0; index < formals.size(); index++)
            fun_env = new ExtendEnv(fun_env, formals.get(index), actuals.get(index), currentDepth + 1);
    ...
    public Value visit(SwitchExp e, Env env) {
        Value val = e.val().accept(this, env);
        
        for (CaseExp c : e.cases()) {
            Value matchResult = c.match_exp().accept(this, env);
            
            if (val.equals(matchResult)) {
                return (Value) c.exp().accept(this, env);
            }
        }
        return (Value) e.default_exp().accept(this, env);
    }
    @Override
    public Value visit(CaseExp e, Env env) {
        return new Value.DynamicError("CaseExp visited outside of Switch context");
    }

    @Override
    public Value visit(DefaultExp e, Env env) {
