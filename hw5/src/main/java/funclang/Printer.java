...
public class Printer {
    ...
    public static class Formatter implements AST.Visitor<String> {
        ...
        public String visit(AST.SwitchExp e, Env env) {
          return "(switch %s %s %s)".formatted(
              e.val().accept(this, env),
              e.cases().stream()
                            .map(opnd -> opnd.accept(this, env))
                            .map(Object::toString)
                            .collect(Collectors.joining(" ")),
              e.default_exp().accept(this,env));
        }

        @Override
        public String visit(AST.CaseExp e, Env env) {
            return "(case %s %s)".formatted(
                e.match_exp().accept(this, env),
                e.exp().accept(this, env));
        }

        @Override
        public String visit(AST.DefaultExp e, Env env) {
            return "(default %s)".formatted(e.exp().accept(this, env));
        }
        // =========================================================
