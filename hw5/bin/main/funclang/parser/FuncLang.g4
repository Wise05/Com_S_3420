...
// Added Switch expression =============================
 switchexp returns [SwitchExp ast] 
          locals [ArrayList<CaseExp> cases = new ArrayList<CaseExp>(); ] :
    '(' Switch 
        val=exp 
        ( e=caseexp { $cases.add($e.ast); } )*
        d=defaultexp
    ')' { $ast = new SwitchExp($val.ast, $cases, $d.ast); }
    ;
 ...
 caseexp returns [CaseExp ast] :
    '(' Case 
        e1=exp
        e2=exp 
    ')' { $ast = new CaseExp($e1.ast, $e2.ast); }
    ;

 defaultexp returns [DefaultExp ast] :
    '(' Default 
        e=exp 
    ')' { $ast = new DefaultExp($e.ast); }
    ;
