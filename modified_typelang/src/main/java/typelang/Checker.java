...
public class Checker implements Visitor<Type, Type> {
  ...
  public Type visit(RefEqExp e, Env<Type> env) {
      ...
      if (!(t1 instanceof RefT) || !(t2 instanceof RefT)) {
          return new ErrorT("The reference equality comparator expects Ref types, found " 
              + t1.toString() + " and " + t2.toString() + " in " + ts.visit(e, null));
      }

      if (!t1.typeEqual(t2)) {
          return new ErrorT("Cannot compare references of different types: " 
              + t1.toString() + " and " + t2.toString() + " in " + ts.visit(e, null));
      }
