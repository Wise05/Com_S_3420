...
public class Evaluator implements Visitor<Value, Value> {
    ...
    public Value visit(RefEqExp e, Env<Value> env) {
      Value.RefVal first = (Value.RefVal) e.first_exp().accept(this, env);
      Value.RefVal second = (Value.RefVal) e.second_exp().accept(this, env);
      return new Value.BoolVal(first.loc() == second.loc());
    }
}
